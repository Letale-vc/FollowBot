...
namespace FollowBot.SimpleEXtensions.CommonTasks
{
    public class LootItemTask : ITask
    {
        ...
        private readonly Stopwatch _logSW = Stopwatch.StartNew();
        ...
        public async Task<bool> Run()
        {
            ...
            var items = CombatAreaCache.Current.Items.FindAll(i =>
                !i.Ignored && !i.Unwalkable &&
                ((FollowBotSettings.Instance.ShouldLootOnlyQuestItem && i.Rarity == Rarity.Quest) ||
                 !FollowBotSettings.Instance.ShouldLootOnlyQuestItem));

            var refPos = FollowBot.Leader != null ? FollowBot.Leader.Position : LokiPoe.Me.Position;

            var validItems = items.FindAll(i =>
                i.Position.AsVector.Distance(refPos) <= FollowBotSettings.Instance.MaxLootDistance);

            var itToFarCount = items.Count - validItems.Count;

            if (validItems.Count == 0)
            {
                if (itToFarCount <= 0 || _logSW.ElapsedMilliseconds <= 5000) return false;
                GlobalLog.Warn(
                    $"[LootItemTask] {itToFarCount} will be ignored B/C they are to far away. (Increase MaxLootDistance or bring the Leader closerto them.)");
                _logSW.Restart();

                return false;
            }

            if (_item == null) _item = validItems.OrderBy(i => i.Position.DistanceSqr).First();
            ...
            {
                GlobalLog.Warn(
                    $"[LootItemTask] {pos} is now to far away. [{pos.Distance}/{FollowBotSettings.Instance.MaxLootDistance}]");
                //_item.Ignored = true;
                _item = null;
                return true;
            }
            ...
            if (pos.Distance > 30 || pos.PathDistance > 34)
            {
                ...
                if (PlayerMoverManager.MoveTowards(pos)) return true;
                ...
                {
                    GlobalLog.Error(
                        $"[LootItemTask] Fail to move to {pos}. item Object is null, removing item from the cache and reevaluating it.");
            ...
            if (attempts > MaxItemPickupAttempts)
            {
                ...
                {
                    var errorName = "[LootItemTask] Fail to pick up the Mirror of Kalandra!!!!!!";
            ...
            if (attempts % 2 == 0) await PlayerAction.DisableAlwaysHighlight();
            ...
            var minTimeout = 400;
            var timeout = Math.Max(LatencyTracker.Average * 2, minTimeout);
        ...
        private static async Task<bool> FastInteraction(WorldItem item)
        {
            ...
            if (LokiPoe.Input.Binding.KeyPickup == LokiPoe.ConfigManager.KeyPickupType.UseHighlightKey)
            {
                ...
                LokiPoe.ProcessHookManager.SetKeyState(LokiPoe.Input.Binding.highlight_combo.Key, -32768,
                    LokiPoe.Input.Binding.highlight_combo.Modifier);
                useHighlight = true;
            ...
            if (LokiPoe.Input.Binding.KeyPickup == LokiPoe.ConfigManager.KeyPickupType.UseBoundKey)
            {
                ...
                LokiPoe.ProcessHookManager.SetKeyState(LokiPoe.Input.Binding.enable_key_pickup_combo.Key, -32768,
                    LokiPoe.Input.Binding.enable_key_pickup_combo.Modifier);
                useBound = true;
                await Wait.SleepSafe(15);
            }
            ...
            for (var i = 1; i < 6; i++)
            {
                point = new Vector2i((int)(label.Coordinate.X + label.Size.X / 7 * i),
                    (int)(label.Coordinate.Y + label.Size.Y / 2));
            ...
            if (!found)
            {
                if (useHighlight)
                {
                    LokiPoe.ProcessHookManager.SetKeyState(LokiPoe.Input.Binding.highlight_combo.Key, 0,
                        LokiPoe.Input.Binding.highlight_combo.Modifier);
                    await Wait.SleepSafe(15);
                }
                ...
                if (useBound)
                {
                    LokiPoe.ProcessHookManager.SetKeyState(LokiPoe.Input.Binding.enable_key_pickup_combo.Key, 0,
                        LokiPoe.Input.Binding.enable_key_pickup_combo.Modifier);
                    await Wait.SleepSafe(15);
                }
