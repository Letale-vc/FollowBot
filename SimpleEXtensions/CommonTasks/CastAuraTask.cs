...
namespace FollowBot
{
    public class CastAuraTask : ITask
    {
        ...
        private static readonly List<int> TemporaryBlacklistedAuras = new List<int>();
        ...
        private static IEnumerable<Skill> AllAuras
        {
            get
            {
                return SkillBar.Skills.Where(skill => !skill.IsVaalSkill && (AuraNames.Contains(skill.Name) ||
                                                                             AuraInternalId.Contains(
                                                                                 skill.InternalId) ||
                                                                             skill.IsAurifiedCurse ||
                                                                             AspectsNames.Contains(skill.Name)));
            }
        }
        ...
        private static IEnumerable<Skill> AllWhitelistedAuras
        {
            get
            {
                return SkillBar.Skills.Where(skill => !skill.IsVaalSkill &&
                                                      TemporaryBlacklistedAuras.All(x => x != skill.Id) &&
                                                      !SkillBlacklist.IsBlacklisted(skill) &&
                                                      (AuraNames.Contains(skill.Name) ||
                                                       AuraInternalId.Contains(skill.InternalId) ||
                                                       skill.IsAurifiedCurse ||
                                                       (FollowBotSettings.Instance.EnableAspectsOfTheAvian &&
                                                        skill.Name == "Aspect of the Avian") ||
                                                       (FollowBotSettings.Instance.EnableAspectsOfTheCat &&
                                                        skill.Name == "Aspect of the Cat") ||
                                                       (FollowBotSettings.Instance.EnableAspectsOfTheCrab &&
                                                        skill.Name == "Aspect of the Crab") ||
                                                       (FollowBotSettings.Instance.EnableAspectsOfTheSpider &&
                                                        skill.Name == "Aspect of the Spider")
                                                      ));
            }
        }
        ...
        public async Task<bool> Run()
        {
            ...
            if (FollowBotSettings.Instance.UseStalkerSentinel)
                if (!LokiPoe.InGameState.SentinelSkillUi.StalkerSentinel.IsActive &&
                    LokiPoe.InGameState.SentinelSkillUi.StalkerSentinel.CanUse)
        ...
        {
            var slotForHidden = AllAuras.First(a => a.IsOnSkillBar).Slot;
            foreach (var aura in auras.OrderByDescending(a => a.Slot))
            {
                if (LokiPoe.Me.IsDead) break;
                if (aura.Slot == -1) await SetAuraToSlot(aura, slotForHidden);
        ...
        private static async Task ApplyAura(Skill aura)
        {
            ...
            {
                if (!await Wait.For(
                        () => !LokiPoe.Me.IsDead && !LokiPoe.Me.HasCurrentAction && LokiPoe.Me.Auras.Any(x =>
                            x.InternalName == "blood_armour" || x.InternalName == "sand_armour"), "aura applying"))
                {
                    ...
                    GlobalLog.Warn(
                        "[CastAuraTask] Also make sure you have blacklisted the auras you dont want to use (Settings-Content-SkillBlacklist).");
                    GlobalLog.Warn(
                        "[CastAuraTask] This error Usually indicate that you have more Auras, slotted, than you can substain with your mana.");
                    GlobalLog.Warn(
                        $"[CastAuraTask] The aura \"{name}\" will not be blacklisted to allow the bot to continue, the aura will be recasted next time you stop/start the bot.");
                    TemporaryBlacklistedAuras.Add(id);
                }
            }
            else
            {
                if (!await Wait.For(() => !LokiPoe.Me.IsDead && !LokiPoe.Me.HasCurrentAction && PlayerHasAura(aura),
                        "aura applying"))
                {
                    if (LokiPoe.Me.IsDead) return;
                    GlobalLog.Warn($"[CastAuraTask] Failed to apply aura \"{name}\".");
        ...
        {
            return LokiPoe.Me.Auras.Any(a =>
                (a.Name.EqualsIgnorecase(auraName) || a.Name.EqualsIgnorecase(auraName + " aura")) &&
                a.CasterId == LokiPoe.Me.Id);
        }
        ...
        public void Start()
        {
            TemporaryBlacklistedAuras.Clear();
        }
